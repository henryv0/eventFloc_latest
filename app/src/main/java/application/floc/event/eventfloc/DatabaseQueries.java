...
import application.floc.event.eventfloc.Events.Attends;
import application.floc.event.eventfloc.Events.Comment;
import application.floc.event.eventfloc.Events.Event;
import application.floc.event.eventfloc.Events.EventType;
import application.floc.event.eventfloc.Events.FollowSociety;
import application.floc.event.eventfloc.Events.HasCategory;
import application.floc.event.eventfloc.Users.Admin;
import application.floc.event.eventfloc.Users.Society;
import application.floc.event.eventfloc.Users.Student;
import application.floc.event.eventfloc.Users.User;
...
public class DatabaseQueries extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String query1 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Careers\")"; //ID = 0
        String query2 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Faculty\")";// id = 1
        String query3 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Interests\")"; //id = 2
        String query4 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Cultural\")";//id = 3
        String query5 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Food\")";//id = 4
        String query6 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Free Food\")";//id = 5
        String query7 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Sports\")";//id = 6
        String query8 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Fundraisers\")";//id = 7
        String query9 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Learning\")";//id = 8
        String query10 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Festivals\")";//id = 9
        String query11 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Performing Arts\")";//id = 10
        String query12 = "INSERT INTO " + TABLE_EVENT_TYPE + "values (\"Political\")";//id = 11
     ...
     * @return
     */
    public void insertEvent(Event event, int eventType) {
        ContentValues cv = new ContentValues();
        ...
        db.insert(TABLE_HAS_CATEGORY, null, cv1);
    ...
    public Society getSocietyUserId(int userId){
        String query = "SELECT u." + USER_ID + ", s."+ SOCIETY_USER_ID + ", s." + SOCIETY_ID + ", s."
                + SOCIETY_NAME + ", s." + SOCIETY_FACULTY + ", s." + SOCIETY_DESC + " from "
                + TABLE_SOCIETY + " s JOIN " + TABLE_USER + " u ON u." + USER_ID + " = s." + SOCIETY_USER_ID
        ...
        if (cursor.moveToFirst()) {
            ...
            society.setUserID(Integer.parseInt(cursor.getString(0)));
            society.setUserType(2);
            society.setSocietyID(Integer.parseInt(cursor.getString(2)));
            society.setSocietyName(cursor.getString(3));
            society.setSocietyFaculty(cursor.getString(4));
            society.setDescription(cursor.getString(5));
...
//--------------------------------------------UPDATE------------------------------------------------
    ...
    public ArrayList<Event> getAllEvents() throws ParseException {
        ...
        // "SELECT  e." + EVENT_ID + ", e." + EVENT_NAME + ", e." + EVENT_SOCIETY_ID + ", s." + SOCIETY_NAME + ", e."
        //   + EVENT_LOCATION + ", e." + EVENT_LINK + ", e." + EVENT_START_TIME + ", e."
        //  + EVENT_END_TIME + ", et." + EVENT_TYPE + " FROM " + TABLE_EVENT + " e JOIN "
        //   + TABLE_HAS_CATEGORY + " h ON e." + EVENT_ID + " = h." + HAS_EVENT_ID
        //   + " JOIN " + TABLE_EVENT_TYPE + " et ON h." + HAS_EVENT_TYPE_ID + " = et."
        //   + EVENT_TYPE_ID + " JOIN " + TABLE_SOCIETY + " s ON e." + EVENT_SOCIETY_ID
        //  + " = s." + SOCIETY_ID + ";";
        //SELECT  e.event_id, e.event_name, e.society_id, s.society_name, e.event_location,
        // e.event_link, e.event_start_time, e.event_end_time, h.event_id, h.event_type_id, et.event_type FROM Event e
        // JOIN Has_Category h ON e.event_id = h.event_id JOIN Event_Type et ON
        // h.event_type_id = et.event_type_id JOIN Society s ON e.society_id = s.society_id;
    ...
    /**
     * Get All events from a particular category
     * @param category
     * @return
     * @throws ParseException
     */
    public ArrayList<Event> getAllEventsCategory(String category) throws ParseException {
        ...
        int eventTypeID = getEventTypeId(category);


        String query2 = "SELECT e." + EVENT_ID + ", e." + EVENT_NAME + ", e."  + EVENT_SOCIETY_ID
                + ", e." + EVENT_LOCATION + ", e." + EVENT_DATE +  ", e." + EVENT_DESC + ", e." + EVENT_LINK + ", e." + EVENT_START_TIME
                + ", e." + EVENT_END_TIME + ", h." + HAS_EVENT_ID + ", h." + HAS_EVENT_TYPE_ID
                + ", et." + EVENT_TYPE_ID + ", et." + EVENT_TYPE + " FROM " + TABLE_EVENT + " e "
                + "JOIN " + TABLE_HAS_CATEGORY + " h ON e." + EVENT_ID + " = h." + HAS_EVENT_ID
                + " JOIN " + TABLE_EVENT_TYPE  + " et ON h." + EVENT_TYPE_ID + " = et."
                + EVENT_TYPE_ID + " WHERE et." + EVENT_TYPE_ID + " = " + eventTypeID + ";";


        Log.d("QUERY 2", "" + query2);

        // "SELECT e.event_id, e.event_name, e.event_society_id, e.event_location, e.event_link," +
        // " e.eventstarttime, e.eventendtime, h.event_id, h.event_type_id, et.event_type_id, et.event_type " +
        //  "FROM EVENTS e JOIN HAS_CATEGORY h on e.event_id = h.event_id " +
        //  "JOIN EVENT_TYPE et ON h.event_type_id = et.event_type_id WHERE et.event_type = "

        Cursor cursor = dq.rawQuery(query2, null);
        int numRows = cursor.getCount();
        ...
        if(cursor.moveToFirst()){
            ...
            Log.d("NO RESULTS", "asd");
        }
        return global;
    }

    /**
     * Find the eventtypeId based on string
     * @param search
     * @return
     */
    public int getEventTypeId(String search){
        SQLiteDatabase db = this.getReadableDatabase();
        int eventTypeId= 0;
        String query = "SELECT * FROM " + TABLE_EVENT_TYPE + ";";
        Cursor c = db.rawQuery(query, null);
        c.moveToFirst();
        for(int i = 0; i<c.getCount(); i++){
            EventType et = new EventType();
            et.setEventTypeID(Integer.parseInt(c.getString(0)));
            et.setEventType(c.getString(1));

            if(et.getEventType().equals(search)){
